...
## Phase 4 — 추가 실제 사용 사례 5편 (입문자 감정 + 복합 기능) `[APPROVED 2026-04-08]`

> 대표님: "테크 교육소설 작가" 정체성 부여 + "40~60대의 희망" 책임 + v2 기획(복합 기능) 승인

### 🛡️ 절대 원칙 (위반 = 재작성)
1. **거짓 정보 제로** — 설치 명령어·에러 메시지·UI·기능 동작 모두 공식 소스 검증
2. **입문자 입장 100%** — 아무것도 모르는 사람이 그대로 따라한다는 마음
3. **감정의 첫 경험** — 낯섦·공포·작은 성공·뿌듯함을 섬세하게
4. **한 편에 복합 기능** — 각 편 3~7개 기능이 자연스럽게 연결

### 편성
| 편 | 제목 | 주인공 | 복합 기능 |
|---|---|---|---|
| 11 | 진숙 여사의 검은 화면 첫 만남 | 진숙(62, 은퇴 교사) | 설치/첫대화/`/help`/`/powerup`/음성/CLAUDE.md |
| 12 | 오사장의 카페, 매일 아침 6시 30분의 기적 | 오사장(54, 카페) | 프로젝트/아티팩트/파일첨부/예약작업/Dispatch/스킬 |
| 13 | 박과장의 첫 실수, 그리고 반드시 지키는 작은 스킬 | 박과장(47, 총무) | `/btw`/`/doctor`/`/rewind`/스킬 만들기/스킬 쓰기/CLAUDE.md |
| 14 | 컴맹 아내, IT 남편보다 먼저 배우다 | 김선영(51, 주부)+남편(53) | 프로젝트/아티팩트/`/fork`/CLAUDE.md/스킬/teleport/파일첨부 |
| 15 | 복순 할머니의 블로그, 논둑길에서 쓴 글 | 복순(68, 농부) | 음성/아티팩트/CLAUDE.md/예약작업/Dispatch/스킬/`/btw` |

### 팀 구성
- 작가 A (`technical-writer`) — 편 11 + 편 12 (첫 경험 + 루틴화)
- 작가 B (`technical-writer`) — 편 13 + 편 14 (실수 복구 + 세대 연결)
- 작가 C (`technical-writer`) — 편 15 (감성 블로그)

### 집필 후 Critic 검수 (본인)
- 설치 명령어·에러 메시지·UI 설명 공식 소스 대조
- 구체적 산출물·Before/After 수치·감정 포인트 체크
- 입문자 따라하기 가능성 검증

### 파일 위치
`content/docs/stories/` 하위:
- `starter-01-jinsook-first-install.md`
- `starter-02-oh-cafe-routine.md`
- `starter-03-park-skill-creation.md`
- `starter-04-couple-it-reversal.md`
- `starter-05-boksun-blog.md`
