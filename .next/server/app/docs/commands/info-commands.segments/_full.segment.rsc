...
12:["$","span","1",{"className":"flex items-center gap-2 min-w-0","children":[["$","span",null,{"style":{"color":"var(--border)","fontSize":"16px"},"children":"/"}],["$","span",null,{"className":"font-medium truncate","style":{"color":"var(--text-primary)","fontSize":"14px"},"children":"정보 확인 명령어"}]]}]
13:["$","$L15",null,{}]
16:T25ef,<blockquote>
<p>📅 최종 업데이트: 2026년 3월 22일</p>
</blockquote>
<h2>정보 확인이 왜 중요한가?</h2>
<p>Claude Code를 사용하면서 궁금한 점들이 많아요:</p>
<ul>
<li>"지금까지 얼마나 썼을까?"</li>
<li>"현재 어떤 모델을 쓰고 있지?"</li>
<li>"컨텍스트가 얼마나 남았지?"</li>
</ul>
<p>이런 정보를 확인하는 명령어들입니다.</p>
<hr>
<h2>정보 확인 명령어 7가지</h2>
<h3>1️⃣ <code>/status</code> — 현재 상태 한눈에 보기</h3>
<pre><code class="language-bash">/status
</code></pre>
<p><strong>역할:</strong> Claude Code의 현재 상태를 모두 표시</p>
<div class="note-circle">
○ 가장 많이 사용하는 정보 확인 명령어입니다
</div>
<p><strong>표시되는 정보:</strong></p>
<ul>
<li>현재 사용 중인 AI 모델</li>
<li>현재 세션 이름</li>
<li>토큰 사용량 (지금까지 사용한 데이터 양)</li>
<li>남은 크레딧 (유료 사용자)</li>
<li>응답 시간 (얼마나 빠른가)</li>
</ul>
...
<ul>
<li>비용이 얼마나 들고 있는지 확인할 때</li>
<li>현재 모델이 맞는지 확인할 때</li>
<li>응답이 느린 이유를 조사할 때</li>
</ul>
<hr>
<h3>2️⃣ <code>/cost</code> — 비용 내역 상세 보기</h3>
<pre><code class="language-bash">/cost
</code></pre>
<p><strong>역할:</strong> 비용 사용 내역을 상세하게 표시</p>
<p><mark>요금 결제를 하는 사용자라면 정기적으로 확인해야 합니다</mark></p>
<p><strong>표시되는 정보:</strong></p>
<ul>
<li>오늘 사용 비용</li>
<li>이번 주 총 비용</li>
...
<ul>
<li>빠른 작업은 <code>claude-haiku</code> 사용</li>
<li><code>/compact</code>로 대화 압축해 토큰 줄이기</li>
<li>불필요한 맥락은 <code>/clear</code>로 제거</li>
</ul>
<hr>
<h3>3️⃣ <code>/usage</code> — 사용량 통계 보기</h3>
<pre><code class="language-bash">/usage
</code></pre>
<p><strong>역할:</strong> 토큰 사용량, 대화 횟수 등의 상세 통계</p>
<p><strong>표시되는 정보:</strong></p>
<ul>
<li>총 토큰 사용량</li>
<li>입력 토큰 vs 출력 토큰 비율</li>
<li>평균 응답 길이</li>
<li>총 대화 횟수</li>
<li>가장 많이 사용한 모델</li>
</ul>
...
<ul>
<li>어떤 모델을 가장 많이 썼는지 알기</li>
<li>평균 응답 시간 확인</li>
<li>비용 최적화 계획 세우기</li>
</ul>
<hr>
<h3>4️⃣ <code>/stats</code> — 종합 통계 보기</h3>
<pre><code class="language-bash">/stats
</code></pre>
<p><strong>역할:</strong> 사용 패턴과 추세를 종합적으로 분석</p>
<p><strong>표시되는 정보:</strong></p>
<ul>
<li>이번 주/달/년 트렌드</li>
<li>가장 활발한 사용 시간</li>
...
<hr>
<h3>5️⃣ <code>/context</code> — 컨텍스트 정보 보기</h3>
<pre><code class="language-bash">/context
</code></pre>
<p><strong>역할:</strong> 현재 대화의 메모리 상태 확인</p>
<div class="note-circle">
...
<li>최근 메시지 개수</li>
<li>총 토큰 수</li>
<li>남은 컨텍스트 윈도우 (더 입력할 수 있는 여유)</li>
</ul>
<p><strong>사용 예시:</strong></p>
<pre><code class="language-bash">/context
...
<p><strong>언제 사용할까?</strong></p>
<ul>
<li>대화가 너무 길어졌을 때 (<code>/compact</code>로 압축 검토)</li>
<li>AI가 이전 내용을 기억하지 못할 때</li>
<li>메모리 정리가 필요할 때</li>
</ul>
<hr>
<h3>6️⃣ <code>/doctor</code> — 진단 및 건강 체크</h3>
<pre><code class="language-bash">/doctor
</code></pre>
<p><strong>역할:</strong> Claude Code의 현재 상태를 진단</p>
<p><mark>문제가 있는지 자동으로 확인해줍니다</mark></p>
...
<p><strong>문제가 있으면:</strong></p>
<pre><code>Network: ✗ Slow (connection latency)
API Response: ✗ Timeout
Suggestion: Check internet connection
</code></pre>
<hr>
<h3>7️⃣ <code>/help</code> — 모든 명령어 도움말 보기</h3>
<pre><code class="language-bash">/help
</code></pre>
<p><strong>역할:</strong> 사용 가능한 모든 명령어의 목록과 설명 표시</p>
<p><strong>표시되는 정보:</strong></p>
<ul>
<li>모든 슬래시 커맨드 목록</li>
<li>각 명령어의 짧은 설명</li>
...
<p><strong>언제 사용할까?</strong></p>
<ul>
<li>명령어 기억이 안 날 때 (가장 흔함)</li>
<li>새로운 기능을 알고 싶을 때</li>
<li>명령어 문법을 확인하고 싶을 때</li>
</ul>
<hr>
<h2>📊 정보 확인 명령어 비교</h2>
<p>어떤 명령어를 언제 쓸까요?</p>
<p>| 명령어 | 용도 | 사용 시기 |
|--------|------|----------|
| <code>/status</code> | 빠른 상태 확인 | 매번 시작할 때 |
| <code>/cost</code> | 비용 상세 보기 | 주 1회 (비용 체크) |
| <code>/usage</code> | 통계 분석 | 월 1회 (패턴 분석) |
| <code>/stats</code> | 추세 분석 | 월 1회 (최적화 검토) |
| <code>/context</code> | 메모리 상태 | 대화가 길어질 때 |
| <code>/doctor</code> | 문제 진단 | 뭔가 안 될 때 |
| <code>/help</code> | 명령어 확인 | 명령어 까먹었을 때 |</p>
<hr>
<h2>💡 쉽게 이해하기</h2>
<h3>정보 확인을 운전면허 검사로 비유하면?</h3>
<p>Claude Code의 정보 확인 명령어는 <strong>차량 검사소의 각종 검진</strong>과 같습니다:</p>
<ul>
<li><strong><code>/status</code></strong> = 간단한 외관 점검 (빨리 끝남)</li>
<li><strong><code>/cost</code></strong> = 유지비 내역 확인</li>
<li><strong><code>/usage</code></strong> = 상세한 정비 기록 확인</li>
<li><strong><code>/stats</code></strong> = 주기적인 성능 테스트</li>
<li><strong><code>/context</code></strong> = 연료/배터리 상태 확인</li>
<li><strong><code>/doctor</code></strong> = 종합 점검 (뭔가 문제 있을 때)</li>
<li><strong><code>/help</code></strong> = 검사 가이드북</li>
</ul>
<h3>예를 들어...</h3>
<p><strong>상황 1: 요금이 걱정될 때</strong></p>
<pre><code class="language-bash">> 이번 달에 얼마나 썼을까?
/cost
→ 이번 달: $12.50 (예상치보다 적음, 안심)
</code></pre>
<p><strong>상황 2: 응답이 느릴 때</strong></p>
<pre><code class="language-bash">> 왜 응답이 이렇게 느려?
/doctor
→ API Response: ✗ Timeout
→ 인터넷 연결 확인 필요
</code></pre>
<p><strong>상황 3: 대화가 너무 길어졌을 때</strong></p>
<pre><code class="language-bash">> AI가 예전 내용을 자꾸 까먹어...
/context
