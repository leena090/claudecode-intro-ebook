...
11:["$","$Lf",null,{"href":"/docs/config","className":"breadcrumb-link shrink-0","children":"설정 & 커스터마이징"}]
12:["$","span","1",{"className":"flex items-center gap-2 min-w-0","children":[["$","span",null,{"style":{"color":"var(--border)","fontSize":"16px"},"children":"/"}],["$","span",null,{"className":"font-medium truncate","style":{"color":"var(--text-primary)","fontSize":"14px"},"children":"settings.json 설정하기"}]]}]
13:["$","$L15",null,{}]
16:T14de,<blockquote>
<p>📅 최종 업데이트: 2026년 3월 22일</p>
</blockquote>
<h2>settings.json이란?</h2>
<p><strong>settings.json</strong>은 클로드 코드의 세부 동작을 제어하는 설정 파일입니다. 마치 스마트폰의 설정 앱에서 밝기, 소리, 알림을 조정하듯이, 당신의 AI 어시스턴트를 맞춤형으로 구성할 수 있습니다.</p>
<hr>
<h2>설정 파일은 어디에 두나?</h2>
<h3>1. 사용자 전체 설정 (글로벌)</h3>
<pre><code>~/.claude/settings.json       ← 모든 프로젝트에 적용
</code></pre>
<h3>2. 프로젝트별 설정</h3>
<pre><code>프로젝트폴더/
...
<p>선택지:</p>
<ul>
<li><code>claude-opus-4-6</code> — 가장 강력한 모델 (추천)</li>
<li><code>claude-sonnet-4-6</code> — 균형잡힌 속도와 성능</li>
<li><code>claude-haiku-4-5</code> — 빠르고 가벼움</li>
</ul>
<hr>
<h3>2. 권한 설정 (Permissions)</h3>
<pre><code class="language-json">{
  "permissions": {
    "mode": "default",
...
<p><strong>3가지 모드:</strong></p>
<ul>
<li><code>default</code> — 매번 물어봄</li>
<li><code>ask</code> — 도구 사용 전 확인 (안전함)</li>
<li><code>allow</code> — 자동으로 허용 (빠름)</li>
</ul>
<hr>
<h3>3. 환경 변수 (Environment)</h3>
<pre><code class="language-json">{
  "env": {
    "PROJECT_NAME": "내_프로젝트",
    "LANGUAGE": "korean",
...
<p><strong>지원되는 이벤트:</strong></p>
<ul>
<li><code>PreToolUse</code> — 도구 실행 전</li>
<li><code>PostToolUse</code> — 도구 실행 후</li>
<li><code>UserPromptSubmit</code> — 사용자 입력 전</li>
</ul>
<hr>
<h2>기본 설정 예시</h2>
<pre><code class="language-json">{
...
<h2>보안을 위한 설정</h2>
<h3>API 키 안전 관리</h3>
<p><mark>절대 settings.json에 API 키를 넣지 마세요!</mark></p>
<p><strong>올바른 방법:</strong></p>
<p>1️⃣ <code>.env</code> 파일에 저장</p>
<pre><code>OPENAI_API_KEY=sk-...
GITHUB_TOKEN=ghp_...
</code></pre>
<p>2️⃣ settings.json에서 참고</p>
<pre><code class="language-json">{
  "apiKeyHelper": {
    "dotenv": ".env"
  }
...
<hr>
<h2>💡 쉽게 이해하기</h2>
<p>settings.json은 <strong>당신의 선호도를 AI에게 알려주는 프로필</strong>입니다.</p>
<ul>
<li><strong>프로필이 없으면:</strong> AI가 매번 같은 질문을 함</li>
<li><strong>프로필이 있으면:</strong> AI가 당신의 취향을 미리 알고 진행</li>
<li><strong>여러 프로필:</strong> 프로젝트마다 다른 설정 사용 가능</li>
</ul>
<p>예를 들어:</p>
<ul>
<li>회사 프로젝트: 강력한 모델 + 엄격한 권한</li>
<li>개인 프로젝트: 빠른 모델 + 자유로운 권한</li>
<li>학습 프로젝트: 균형잡힌 모델 + 중간 권한</li>
</ul>
<hr>
<h2>JSON 형식 주의</h2>
<p><strong>틀린 예시:</strong></p>
<pre><code class="language-json">{
  "model": "claude-sonnet-4-6"
  "permissions": {}  ← 쉼표 빠짐!
}
