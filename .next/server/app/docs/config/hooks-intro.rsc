...
12:["$","span","1",{"className":"flex items-center gap-2 min-w-0","children":[["$","span",null,{"style":{"color":"var(--border)","fontSize":"16px"},"children":"/"}],["$","span",null,{"className":"font-medium truncate","style":{"color":"var(--text-primary)","fontSize":"14px"},"children":"Hooks로 자동화하기"}]]}]
13:["$","$L15",null,{}]
16:T19ee,<blockquote>
<p>📅 최종 업데이트: 2026년 3월 22일</p>
</blockquote>
<h2>Hooks란?</h2>
<p><strong>Hooks</strong>는 특정 상황이 발생했을 때 자동으로 무언가를 실행하도록 하는 설정입니다. 마치 자동 수도꼭지처럼, 손을 가져다 대면 자동으로 물이 나오듯이, 정해진 이벤트가 발생하면 자동으로 동작합니다.</p>
<hr>
<h2>주요 Hooks 종류</h2>
<h3>1. PreToolUse — 도구 사용 전</h3>
...
<ul>
<li>AI가 파일을 읽기 전</li>
<li>터미널 명령을 실행하기 전</li>
<li>검증/확인이 필요할 때</li>
</ul>
<hr>
<h3>2. PostToolUse — 도구 사용 후</h3>
<pre><code class="language-json">{
  "hooks": {
    "PostToolUse": {
      "handler": "command",
...
<li>파일 수정이 완료된 후</li>
<li>터미널 명령이 끝난 후</li>
<li>결과를 저장하거나 알림을 보낼 때</li>
</ul>
<hr>
<h3>3. UserPromptSubmit — 사용자 입력 후</h3>
<pre><code class="language-json">{
  "hooks": {
    "UserPromptSubmit": {
      "handler": "command",
...
<p><strong>언제 실행?</strong></p>
<ul>
<li>사용자가 메시지를 보낸 후</li>
<li>AI가 응답하기 전</li>
<li>입력 기록, 검증, 필터링에 사용</li>
</ul>
<hr>
<h2>3가지 Handler 타입</h2>
<h3>1. command — 셸 명령 실행</h3>
<pre><code class="language-json">{
  "handler": "command",
...
<hr>
<h2>⚠️ Hooks 사용 시 주의사항</h2>
<h3>1. Exit Code 확인</h3>
<pre><code class="language-json">{
  "handler": "command",
  "command": "rm file.txt"
}
...
<ul>
<li>Exit code 0 = 성공 → 계속 진행</li>
<li>Exit code != 0 = 실패 → 작업 중단</li>
</ul>
<hr>
<h3>2. 성능 고려</h3>
<pre><code class="language-json">❌ 시간 오래 걸리는 작업
{
...
<hr>
<h3>3. 순환 참조 방지</h3>
<pre><code class="language-json">❌ 위험한 설정
{
  "PreToolUse": {
    "handler": "command",
...
<hr>
<h2>💡 쉽게 이해하기</h2>
<p>Hooks는 <strong>스마트홈의 자동화</strong>와 같습니다.</p>
<ul>
<li><strong>Hook이 없으면:</strong> 집에 들어올 때마다 불을 켜고 에어컨을 켜고...</li>
<li><strong>Hook이 있으면:</strong> 집에 들어오면 자동으로 불이 켜지고 에어컨이 켜짐</li>
<li><strong>복잡한 Hook:</strong> 집에 들어올 때 조명을 70% 밝기로 설정하고 음악을 틀기</li>
</ul>
<p>마찬가지로:</p>
<ul>
<li>AI가 파일을 수정할 때마다 자동 테스트</li>
<li>코드 작성 후 자동 포맷팅</li>
<li>깃에 자동 커밋</li>
</ul>
<hr>
<h2>Hooks 문제 해결</h2>
<h3>문제: Hook이 실행되지 않음</h3>
<pre><code>확인 사항:
1. Hook 이름 맞는가? (대소문자 구분)
2. JSON 형식이 맞는가?
3. command 경로가 맞는가?
</code></pre>
<h3>문제: Hook이 너무 자주 실행됨</h3>
<pre><code>해결책: 조건부 실행
"command": "[[ $file_path == *.py ]] &#x26;&#x26; python format.py"
</code></pre>
<hr>
<h2>다음 단계</h2>
<p>Hooks로 자동화했다면, 이제 <strong>MCP 서버 연결</strong>로 더 강력한 기능을 추가해봅시다!</p>
14:["$","div",null,{"className":"max-w-7xl mx-auto px-5 sm:px-8 py-8 flex gap-10","children":[["$","aside",null,{"className":"hidden lg:block w-60 shrink-0","children":["$","div",null,{"className":"sticky top-20","children":[["$","div",null,{"className":"flex items-center gap-2 mb-4","children":[["$","div",null,{"className":"icon-box w-6 h-6 rounded-md text-sm","children":"🔧"}],["$","h3",null,{"className":"text-xs font-semibold uppercase tracking-wider","style":{"color":"var(--text-muted)"},"children":"설정 & 커스터마이징"}]]}],["$","nav",null,{"className":"space-y-0.5","children":[["$","$Lf","claude-md",{"href":"/docs/config/claude-md","className":"sidebar-link ","children":"CLAUDE.md 작성 가이드"}],["$","$Lf","settings-json",{"href":"/docs/config/settings-json","className":"sidebar-link ","children":"settings.json 설정하기"}],["$","$Lf","permissions-guide",{"href":"/docs/config/permissions-guide","className":"sidebar-link ","children":"권한 시스템 이해하기"}],["$","$Lf","hooks-intro",{"href":"/docs/config/hooks-intro","className":"sidebar-link sidebar-link-active","children":"Hooks로 자동화하기"}],["$","$Lf","mcp-setup",{"href":"/docs/config/mcp-setup","className":"sidebar-link ","children":"MCP 서버 연결하기"}]]}],["$","div",null,{"className":"mt-4 pt-3","style":{"borderTop":"1px solid var(--border)"},"children":["$","$Lf",null,{"href":"/docs/config","className":"breadcrumb-link flex items-center gap-1 text-xs","children":[["$","svg",null,{"className":"w-3 h-3","fill":"none","viewBox":"0 0 24 24","strokeWidth":1.5,"stroke":"currentColor","children":["$","path",null,{"strokeLinecap":"round","strokeLinejoin":"round","d":"M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18"}]}],"전체 목록"]}]}]]}]}],["$","main",null,{"className":"min-w-0 flex-1 max-w-3xl","children":[["$","div",null,{"className":"mb-8","children":[["$","h1",null,{"className":"text-2xl sm:text-[1.75rem] font-bold mb-2.5","style":{"color":"var(--text-primary)","letterSpacing":"-0.015em"},"children":"Hooks로 자동화하기"}],["$","p",null,{"className":"text-base","style":{"color":"var(--text-secondary)"},"children":"특정 상황에서 자동으로 실행되는 작업 설정하기"}],["$","div",null,{"className":"flex flex-wrap gap-1.5 mt-4","children":[["$","span","자동화",{"className":"text-xs px-2.5 py-1 rounded-lg font-medium","style":{"background":"var(--accent-soft)","color":"var(--accent-text)"},"children":"자동화"}],["$","span","hooks",{"className":"text-xs px-2.5 py-1 rounded-lg font-medium","style":{"background":"var(--accent-soft)","color":"var(--accent-text)"},"children":"hooks"}],["$","span","이벤트",{"className":"text-xs px-2.5 py-1 rounded-lg font-medium","style":{"background":"var(--accent-soft)","color":"var(--accent-text)"},"children":"이벤트"}]]}]]}],["$","div",null,{"className":"markdown-body","dangerouslySetInnerHTML":{"__html":"$16"}}],"$L17"]}]]}]
