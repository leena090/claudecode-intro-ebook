...
11:["$","$Lf",null,{"href":"/docs/advanced","className":"breadcrumb-link shrink-0","children":"고급 기능"}]
12:["$","span","1",{"className":"flex items-center gap-2 min-w-0","children":[["$","span",null,{"style":{"color":"var(--border)","fontSize":"16px"},"children":"/"}],["$","span",null,{"className":"font-medium truncate","style":{"color":"var(--text-primary)","fontSize":"14px"},"children":"샌드박스 & 보안"}]]}]
13:["$","$L15",null,{}]
16:T20cd,<blockquote>
<p>📅 최종 업데이트: 2026년 3월 22일</p>
</blockquote>
<h2>샌드박스 &#x26; 보안이란?</h2>
<p><strong>샌드박스</strong>는 AI가 할 수 있는 행동을 제한하는 안전 영역입니다. <strong>보안</strong>은 당신의 민감정보(암호, API 키, 개인정보)를 보호하는 것입니다. 마치 은행의 보안 시스템처럼, 허가된 것만 접근하고 나머지는 차단합니다.</p>
<hr>
<h2>샌드박스 모드</h2>
<h3>샌드박스 모드란?</h3>
<p>AI가 할 수 있는 작업을 제한하는 모드입니다.</p>
<pre><code>일반 모드:
AI가 모든 파일에 접근 가능
위험할 수 있음
...
<pre><code class="language-python"># Python에서 사용
import os
...
<hr>
<h4>3. .gitignore에 추가</h4>
<pre><code>.env
.env.local
secrets.json
private/
</code></pre>
<p>→ Git에 민감정보가 올라가지 않음</p>
<hr>
<h2>파일 접근 제어</h2>
<h3>금지된 폴더 설정</h3>
<pre><code class="language-json">{
...
<p><strong>로그 내용:</strong></p>
<ul>
<li>언제: 2026-03-22 14:30:00</li>
<li>누가: claude (AI)</li>
<li>뭘 했는가: file:write /Users/mylee/Desktop/app.py</li>
<li>성공/실패: Success</li>
</ul>
<hr>
<h3>로그 보기</h3>
<pre><code class="language-bash">/audit:log
</code></pre>
<p>→ 최근 50개 작업 기록 표시</p>
<hr>
<h2>보안 체크리스트</h2>
...
<hr>
<h3>팀 감시 및 승인</h3>
<pre><code class="language-bash">/team:approval-required

→ 모든 파일 수정이 리더의 승인 필요
</code></pre>
<hr>
<h2>💡 쉽게 이해하기</h2>
<p>샌드박스와 보안은 <strong>집의 보안 시스템</strong>과 같습니다.</p>
<p><strong>보안 없이:</strong></p>
<ul>
<li>손님이 모든 방 접근 가능</li>
<li>귀중품 도난 위험</li>
<li>프라이버시 침해</li>
</ul>
<p><strong>보안으로:</strong></p>
<ul>
<li>손님은 거실만 접근</li>
<li>침실, 서재는 잠금</li>
<li>귀중품 안전</li>
...
<p>마찬가지로:</p>
<ul>
<li><strong>샌드박스:</strong> AI는 공개 폴더만 접근</li>
<li><strong>민감정보 보호:</strong> 암호, 키는 .env에만 저장</li>
<li><strong>감시 로그:</strong> 누가 뭘 했는지 기록</li>
</ul>
<hr>
<h2>실전 보안 설정</h2>
<h3>완전한 보안 설정 예시</h3>
...
<hr>
<h2>보안 팁</h2>
<h3>팁 1: 정기적으로 권한 검토</h3>
<pre><code class="language-bash">/security:audit    ← 월 1회 보안 감시
</code></pre>
<hr>
<h3>팁 2: 강력한 암호 사용</h3>
...
<hr>
<h3>팁 3: 퍼블릭 Wi-Fi에서는 VPN 사용</h3>
<pre><code class="language-bash">민감한 작업은 VPN으로 보호된 네트워크에서만
</code></pre>
<hr>
<h2>보안 사고 대응</h2>
<h3>API 키 유출 시</h3>
<pre><code class="language-bash">1. 즉시 유출된 키 비활성화
2. 새로운 키 발급
3. .env 파일 업데이트
4. 모든 서버 재시작
5. 감사 로그 확인
