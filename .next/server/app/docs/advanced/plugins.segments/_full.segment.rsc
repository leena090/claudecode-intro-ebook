...
12:["$","span","1",{"className":"flex items-center gap-2 min-w-0","children":[["$","span",null,{"style":{"color":"var(--border)","fontSize":"16px"},"children":"/"}],["$","span",null,{"className":"font-medium truncate","style":{"color":"var(--text-primary)","fontSize":"14px"},"children":"플러그인 & 스킬 시스템"}]]}]
13:["$","$L15",null,{}]
16:T1b08,<blockquote>
<p>📅 최종 업데이트: 2026년 3월 22일</p>
</blockquote>
<h2>플러그인 &#x26; 스킬이란?</h2>
<p><strong>플러그인</strong>은 클로드의 기능을 확장하는 추가 도구입니다. <strong>스킬</strong>은 자주 반복하는 작업을 간단한 명령어로 실행할 수 있게 저장한 것입니다. 마치 스마트폰에 앱을 설치하듯이, 클로드에도 새로운 기능을 추가할 수 있습니다.</p>
<hr>
<h2>스킬 (Skill) 이해하기</h2>
<h3>스킬의 개념</h3>
<pre><code>일반적인 방식:
...
<h2>스킬 만들기</h2>
<h3>단계 1: 스킬 정의</h3>
<pre><code class="language-bash">당신: "/skill-create make-calculator"
</code></pre>
<h3>단계 2: 스킬 상세 작성</h3>
<pre><code class="language-markdown"># make-calculator
...
<hr>
<h3>단계 3: 스킬 사용</h3>
<pre><code class="language-bash">/make-calculator
</code></pre>
<p>→ 자동으로 계산기 코드 생성!</p>
<hr>
<h2>실전 스킬 예시</h2>
...
<hr>
<h2>플러그인 (Plugin) 이해하기</h2>
<h3>플러그인의 개념</h3>
<pre><code>스킬: 하나의 작업을 간단하게
플러그인: 완전한 기능을 추가 (더 복잡함)
</code></pre>
<hr>
<h2>플러그인 설치</h2>
<h3>공식 플러그인 설치</h3>
<pre><code class="language-bash">claude plugin:install github-helper
</code></pre>
<p>→ GitHub 관련 기능 추가</p>
<hr>
<h3>커스텀 플러그인 만들기</h3>
<h4>폴더 구조</h4>
<pre><code>~/.claude/plugins/
└── my-translator/
    ├── manifest.json
    ├── plugin.js
    └── config.json
...
<h2>스킬과 플러그인 관리</h2>
<h3>설치된 스킬 목록 보기</h3>
<pre><code class="language-bash">/skill:list
</code></pre>
...
<hr>
<h3>설치된 플러그인 목록 보기</h3>
<pre><code class="language-bash">/plugin:list
</code></pre>
<hr>
<h3>스킬/플러그인 제거</h3>
<pre><code class="language-bash">/skill:remove summarize-file
...
<hr>
<h2>커뮤니티 스킬/플러그인 활용</h2>
<h3>마켓플레이스에서 찾기</h3>
<pre><code class="language-bash">/skill:search resume
</code></pre>
<p>→ 이력서 작성 관련 스킬 검색</p>
<hr>
<h3>인기 플러그인</h3>
<pre><code>1. GitHub Helper      - Git 명령 자동화
...
<hr>
<h2>💡 쉽게 이해하기</h2>
<p>스킬과 플러그인은 <strong>포토샵의 필터와 확장</strong>과 같습니다.</p>
<p><strong>필터 (스킬):</strong></p>
<ul>
<li>클릭 한번으로 이미지 처리</li>
<li>자주 쓰는 기능</li>
<li>간단함</li>
</ul>
...
<p>마찬가지로:</p>
<ul>
<li><strong>스킬:</strong> 자주 반복하는 작업을 한 명령어로</li>
<li><strong>플러그인:</strong> 완전히 새로운 기능 추가</li>
</ul>
<hr>
<h2>스킬 및 플러그인 팁</h2>
<h3>팁 1: 시간이 드는 작업을 스킬화</h3>
<pre><code>반복되는 작업들:
✓ 보고서 작성
✓ 코드 검토
✓ 테스트 케이스 생성
...
<hr>
<h3>팁 2: 팀과 스킬 공유</h3>
<pre><code class="language-bash">/skill:share code-review   # 팀원들과 공유
</code></pre>
<hr>
<h3>팁 3: 스킬 체이닝</h3>
<pre><code>스킬 1: /format
스킬 2: /security
스킬 3: /test
