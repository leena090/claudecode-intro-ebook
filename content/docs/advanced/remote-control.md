...
`--remote` 플래그를 사용하면 터미널 대신 브라우저에서 접근할 수 있는 웹 세션으로 Claude Code를 시작합니다:
...
# 결과:
# 세션이 시작됐습니다. 아래 URL로 접속하세요:
# https://claude.ai/session/abc123xyz
```

이 URL을 공유하면 팀원이나 다른 기기에서 같은 세션에 접속할 수 있습니다.

---

## 세션 이전하기 (--teleport)

현재 진행 중인 세션을 다른 기기로 넘길 때는 `--teleport` 플래그를 사용합니다:

```bash
# 현재 세션을 이전 가능한 링크로 생성
claude --teleport
...
iOS와 Android용 Claude 앱에서 진행 중인 세션을 확인하고 간단한 지시를 보낼 수 있습니다:

1. 스마트폰에서 Claude 앱 실행
2. "세션" 탭에서 현재 활성 세션 목록 확인
3. 세션을 탭하면 진행 상황을 볼 수 있고, 간단한 메시지 전송도 가능

> 복잡한 코딩 작업은 데스크톱에서, 진행 상황 확인이나 간단한 지시는 모바일에서 처리하는 방식으로 활용하면 편리합니다.

---

## 예약 실행 (cron 방식)

특정 시간이나 주기로 Claude Code 작업을 자동 실행할 수 있습니다. `/schedule` 명령어를 사용합니다:

```bash
# 매일 오전 9시에 실행
/schedule "0 9 * * *" "git log --oneline -20을 분석해서 일일 요약 만들어줘"

# 매주 월요일 오전 8시에 실행
/schedule "0 8 * * 1" "이번 주 할 일 목록을 정리해줘"
```

등록된 스케줄은 `/schedule list`로 확인하고, `/schedule delete [id]`로 삭제합니다.
