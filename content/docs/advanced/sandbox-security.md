...
<div class="note-star">
★ <strong>공식 발표 기준</strong> — 2026-05-24 공식 문서(<a href="https://code.claude.com/docs/en/sandbox-environments" target="_blank">sandbox-environments</a>) 내용 반영. <code>[공]</code>
</div>
...
목적에 따라 6가지 방식 중 하나를 선택할 수 있어요:

| 방식 | 격리 범위 | Docker 필요 | 설정 난이도 |
|------|----------|:-----------:|:-----------:|
| **샌드박스 Bash 도구** (`/sandbox`) | Bash 명령어만 | ❌ | ⭐ 최소 |
| **샌드박스 런타임** | Claude Code 프로세스 전체 | ❌ | ⭐⭐ 낮음 |
| **Dev 컨테이너** | 전체 개발 환경 | ✅ | ⭐⭐⭐ 보통 |
| **커스텀 컨테이너** | 전체 개발 환경 | ✅ | ⭐⭐⭐⭐ 다소 높음 |
| **가상 머신 (VM)** | 운영체제 전체 | ❌ | ⭐⭐⭐⭐⭐ 높음 |
| **Claude Code 웹 버전** | 전체 OS (Anthropic 호스팅) | ❌ | 없음 |

**상황별 추천:**

- 📱 **일상 작업 중 허가 요청 줄이고 싶을 때** → 샌드박스 Bash 도구 (`/sandbox`)
- 🤖 **사람 없이 Claude 혼자 장시간 실행** → 샌드박스 런타임
- 👥 **팀 전체가 같은 환경에서 일하게 하고 싶을 때** → Dev 컨테이너
- 🔒 **검증 안 된 코드를 안전하게 평가** → 가상 머신
- 🚀 **로컬 설치 없이 바로 시작** → Claude Code 웹 버전

> 🍱 **비유**: 조리사에게 칼을 줄 때 — 믿는 조리사라면 그냥 줘도 되고(제한 없음), 인턴이라면 잘리지 않는 칼을 주거나(샌드박스 Bash), 주방 안에서만 쓰게 하거나(샌드박스 런타임), 별도 조리실에 가둘 수도 있어요(VM). 상황에 맞게 골라요.
